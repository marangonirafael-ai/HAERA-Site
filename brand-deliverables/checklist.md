...
Marque `- [ ]` para `- [x]` conforme for recebendo e conferindo cada item da agência. Este arquivo é mantido por você manualmente; para uma checagem automática do que já está nas pastas, rode `python3 validar_entregaveis.py` (gera um relatório à parte, sem mexer neste arquivo).
...
- [ ] **Códigos de cor completos (HEX, RGB, CMYK, Pantone)** — formato `.pdf, .ai, .txt, .docx` — pasta `02_Cores/01_codigos_cor/` — ESSENCIAL
...
- [ ] **Nome das fontes oficiais (título e texto)** — formato `.pdf, .docx, .txt` — pasta `03_Tipografia/01_especificacao_fontes/` — ESSENCIAL
...
- [ ] **Informação de fonte alternativa (gratuita)** — formato `.pdf, .docx, .txt` — pasta `03_Tipografia/03_fonte_alternativa/` — recomendado
...
- [ ] **Manual em formato editável** — formato `.indd, .ai` — pasta `04_Manual_de_Marca/02_manual_editavel/` — recomendado
      _Assim você atualiza o manual no futuro sem depender da agência original._
- [ ] **Guia de tom de voz e linguagem** — formato `.pdf, .docx` — pasta `04_Manual_de_Marca/03_guia_tom_de_voz/` — recomendado
      _Fundamental para redes sociais, atendimento e conteúdo consistente._

## 5. Arquivos Digitais e de Aplicação

- [ ] **Templates de redes sociais (posts, stories, capa)** — formato `.psd, .ai, .pdf, .txt, .url` — pasta `05_Aplicacoes/01_redes_sociais/` — recomendado
...
- [ ] **Comprovante/licença de fontes pagas usadas** — formato `.pdf` — pasta `06_Documentos_Legais/02_licenca_fontes/` — recomendado
...
- [ ] **Racional criativo / apresentação do conceito da marca** — formato `.pdf, .pptx` — pasta `06_Documentos_Legais/04_racional_criativo/` — recomendado
